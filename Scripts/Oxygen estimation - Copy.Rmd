...
** Use the automatic trim for the ramping period
** For those individuals with weird traces: decide if to drop them(consider potential leaking of gas or bad sealing: eg, 213, 141)
** Compare the logtime of the topt or ropt with the inflection points and see if they match
** Merge datasets and create a single file for statistical analyses
** clean code, add notes and leave only the final version of the working code on this file

* **For Activation energy:**

** Correct the calculation based on my discussion with John on Friday (Oct 7th)
...
5. Finally, it add the oxygen data (based on the calculations of the spreadsheet that Jon DeLong provided me)  
...
r2 = function(data) {
fit = lm(oxymgl~poly(logtime, 10, raw=TRUE), data = data) #fitting the model
...
r3 = function(data) {
     fit = lm(oxymgl~poly(logtime, 10, raw=TRUE), data = data) #fitting the model
     id<-data[1,11] #Getting ID for each individual
...
plot(d7f.data$logtime, d7f.data$y1*-1, pch=19, xlab='Logtime', ylab='Oxygen(mg/l)', main=paste("First derivative: Ind ", id), cex.lab=1.5, cex.axis=1.25, ylim=c(-0.04,0.04)) #plotting the first derivative
dev.off()
}
...
mypath <- file.path("Outputs/Second derivative plots/",paste("Individual_", id, ".jpg", sep = ""))
 ...
 jpeg(file = mypath, width=20, height=12, units="cm", res=300)
plot(d7s.data$logtime, d7s.data$y1, pch=19, xlab='Logtime', ylab='Oxygen(mg/l)', main=paste("Second derivative: Ind", id), cex.lab=1.5, cex.axis=1.25, ylim=c(-0.004,0.004)) #plots the second derivative
...
> To see if the Ropt and topt above where extracted from curves that looked ok, here we plot all the curves for just the period where the ramping happened. If any trace seem off, we can adjust the trimming for a better capture of the real ropt and topt. 
...
indiv<-derivs%>%
  ...
  filter(between(first.logtime, 30, 50)) # %>% #Triming the timeframe to the approximate relevant times (based on plot)
derivstrim<- bind_rows(derivstrim, indiv)
...
indiv<-derivs%>%
  filter(id=="") %>% #filtering to each individual
  filter(between(first.logtime, 30, 90)) # %>% #Triming the timeframe to the approximate relevant times (based on plot)
derivstrim<- bind_rows(derivstrim, indiv)
...
## Replacing old values for selected individuals MODIFY
```{r}
dat<-read.csv("Outputs/ToptandRopts.csv")
