...
1. Wrangle the oxygen and temperature data to create the oxygen consumption rate per temperature for all individuals exposed to the thermolimit respirometry trials.

2. Obtain the estimates of metrics relevant to the physiological responses to temperature (Topt, Ropt, Activation energy)
...
> This is what this part of the code does, per trial:  

1. Prepares the files associated with the trial and brings them into R
...
# Estimating rates of oxygen consumptions: using loops to fit polynomial functions for all individuals 
...
# Estimating preliminary values of physiological tolerance 

## Ropt and Topt for all individuals
...
toptfinal<-oxall %>%
  ...
  rename(logtime=logtime.x, #renaming some variables for simplification
         ...
         ropt2=first2) %>%
  ungroup()%>%
  mutate(stage=fct_relevel(stage, "Protonymph", "Deutonymph", "Tritonymph",  "Male", "Female")) #changing the order of the levels within the factor "stage" 
...
# Checking data again after preliminary estimations of physiological values
...
# Estimating final values of physiological tolerance
...
energyfinal<-oxall %>%
  ...
  right_join(energy, by=c("ind"="ind")) %>%
   ungroup()%>%
  mutate(stage=fct_relevel(stage, "Protonymph", "Deutonymph", "Tritonymph", "Female", "Male" )) %>%
  arrange(stage)
...
There is no difference (diff=0) as it should be. This means there is a match between the topt and the maximum point of the estimation of the slope line that leads to the activation energy. 
