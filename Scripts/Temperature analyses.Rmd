...
In this script I achieve the following:

1. Obtain the final temperature values to then estimate the oxygen consumed by pseudoscorpions in the thermolimit respirometry 

    + I start by reading and structuring the temperature records from both the external HOBO loggers used in the thermolimit respirometry trials. Then I also take the temperatures recorded by hand by both, the thermocouple external sensor, and the recorded temperature by the actual thermal block where the trials took place.And I merge it into a single file to explore the relationship between the different temperature sensors. 

    + I then choose the best/more reliable one to use the most accurate temperature data for the oxygen estimation (separate script).

    + I then used the equation of the line to be able to predict the temperature in the thermal block (the chosen temperature to use) based of the loggers temperatures (for which I have more constant data collected).

    + I use this equation to correct the temperatures of all the trials for the thermolimit respirometry and so be able to further estimate oxygen consumption (separate script)

2. Estimate the heat rigor or CTmax for the pseudoscorpions exposed to the same temperature treatment used in the thermolimit respirometry trials, but scored through behavioral observations (i.e. the point in time and temperature at which they stop moving).

    + For this I used similar steps to the described above.
...
summarize(tempcorr2=mean(tempcorr),
  ...
  right_join(beh)%>%
  ungroup()%>%
  mutate(stage=fct_relevel(stage, "Tritonymph",  "Male", "Female"))%>%
...
meanrigtime
#the Topt is 44.25 (±0.16, SE) across all life stages (Based on the estimations done in a separate script)
#Mean logtime topt: 68.62526 +/- 0.56(standard error) (Based on the estimations done in a separate script)
```


## Plotting to assess the variability of the heat rigor estimates
```{r}
...
> This shows that the estimate heat rigor is congruent with what one would expect: it should happen after and at a higher temperature than the Topt or Tmetmax. However, since a few individuals moved around and climb to the walls of the well in the thermal block, potentially being exposed to slighlty different temperatures, I wanted to see if removing them would change the data significantly:
...
```
> These estimates are lower but more conservative (since all individuals stayed in the same place within the well), so I decided to use this estimates as the final heat rigor data


### Plotting of the final heat rigor data
