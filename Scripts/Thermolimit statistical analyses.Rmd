...
## Checking the ramping rate acrooss trials
```{r}
ggplot(dat, aes(x=ramp, y=tempcorr, color=as.factor(trial)))+geom_point()
```

```{r}
r1<-lm(ramp~stage,data=dat)
summary(r1)
```
...
coefTable <- summary(t1b2, ddf = "Kenward-Roger")$coefficients
coefTable
...
#Comparing AIC values to see which structure of the random factors worked better
aic <- sapply(list(t1, t1b,t1c, t2), AIC)
aic <- data.frame(Model = c("t1", "t1b", "t1c", "t2"), AIC = format(aic, digits = 5)) %>%
    arrange(AIC)
aic

#Seems like t1b has the best structure of the random effects, but t2 is not too far behind
...
### Keeping only ramping as a random factor
```{r}
...
### Checking assumptions
```{r}
check_assumptions(t4)
hist(t2$residuals, main = "Residual Histogram") #it doesnt look too bad
ols_test_normality(t2) #data is  normal
# Still  normal
```
...
predicted.t4 <- augment(t4, newdata = nd, se_fit=TRUE)
stats.t4<-glance(t4)


# calculate the lower and upper 95% confidence limits on the mean
tcrit <- qt(0.975, df = stats.t4$df.residual)
predicted.t4$lwr <- with(predicted.t4, .fitted - tcrit * .se.fit) # 95% confidence interval as estimated when doing the continuous line
predicted.t4$upr <- with(predicted.t4, .fitted + tcrit * .se.fit)
predicted.t4$lwr2 <- with(predicted.t4, .fitted - 1.96 * .se.fit)# 95% confidence interval as estimated when doing the vcategorical variables in Drew's class
predicted.t4$upr2 <- with(predicted.t4, .fitted + 1.96 * .se.fit)
predicted.t4$lwr3 <- with(predicted.t4, .fitted -  .se.fit) #Using just the standard error
predicted.t4$upr3 <- with(predicted.t4, .fitted + .se.fit)


# Plot with prediction trends, confidence intervals and data
t5plot<-ggplot(dat2b, aes(x = stage))+ 
  geom_point(aes(y = tempcorr, color=stage), size = 3, shape=1)  + 
  geom_point(aes(y = .fitted, color=stage),color = "black", size = 2, data = predicted.t4, position=position_nudge(x=0.1), width=0.2) + 
  geom_errorbar(aes(x=stage, ymin = lwr, ymax = upr, group=stage), color = "black", data = predicted.t4, inherit.aes = FALSE, position=position_nudge(x=0.1), width=0.2) + theme_classic()+ theme(text = element_text(size = 15))+ 
  xlab("Stage")+
  ylab("Thermal optima (Celsius)")
t5plot
