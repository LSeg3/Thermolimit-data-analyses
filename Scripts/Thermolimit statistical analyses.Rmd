...
knitr::opts_knit$set(root.dir = "C:/Users/laus1/Documents/UNL/Publications/Ch1 Thermal tolerance/Thermolimit data analyses")
getwd()
```

# Description
In this script, I perform the statistical analyses pertaining the ecophysiological data for *Dactylochelifer silvestris* for my PhD dissertation work, that is also currently in preparation for publication as well. This script builds on the work done by 3 other prior scripts, and uses data estimated and organized in those (Temperature analysis, Oxygen estimation and Environment vs performance).  

This scripts walks you through my statistical analysis process, in which I explore different models to establish the model that best explained the data. I start by doing an analysis to **compare the Topt** (which in my work I ended up labeling as the TMetMax, or maximum tolerated temperature, though others might call this the Thermal optima) between stages and then, also by mass, since the stages have distinct masses (4). 

I then do a separate sets of analysis to assess whether there is a statistically significant **difference between the Ropt (5)** (rate optima) between stages and mass. In this part, I also use this data to estimate the **Metabolic scaling component (5.6)**

I then assess whether the **Activation energy (6)** also varies by stage and/or mass, and visualize the **comparison of environmental conditions to physiological limits (7)**, including the distribution of the Thermal safety margins in current and future conditions. 

At the end of this script, there is also code used to create some example, explanatory plots and final figures used for my dissertation 
# Libraries
```{r}
library(lme4)
library(dplyr)
...
# Preliminary set-up and data wrangling 
...
## Calling data for Topt and Ropt
...
dat<-dat%>%
  ...
  mutate(stage=fct_relevel(stage, "Female", "Protonymph", "Deutonymph", "Tritonymph",  "Male")) %>%
  arrange(stage) %>%
  mutate(date=fct_relevel(date, "2022-06-23", "2022-06-21", "2022-06-22", "2022-06-24")) %>%
  arrange(date)
...
av.weight<-mean(dat$weight)
av.weight


av.ropt<-mean(dat$ropt2)
av.ropt
```
## Exploring the data
...
av.weight<-mean(dat2$weight)
av.weight


av.ropt<-mean(dat2$ropt2)
av.ropt

se.ropt<-sd(dat2$ropt2)/length(dat2$ropt2)
se.ropt


adults<-dat2%>% filter(stage=="Female" | stage=="Male")
mean(adults$weight)
...
## Plotting Topt and heat rigor (from the Temperature analyses script) together

```{r}
#NOTE: for this figures, I used all the individuals exposed to theheat rigor data, undependenlty if they were at the bottom of the well or not. 
heatrig<-read.csv("Outputs/heatrigor.csv")

heatrig$type<-"CTmax"
dat2b$type<-"TMetMax"

tmax<-dat2b %>%
  select(stage, tempcorr, type) %>%
  rename (temp=tempcorr)

rigor<-heatrig %>%
  select(stage, tempcorr2,type) %>%
  rename(temp=tempcorr2)


plotdata<-bind_rows(tmax,rigor) %>%
  mutate(stage=fct_relevel(stage, "Protonymph", "Deutonymph", "Tritonymph",  "Male", "Female"))  %>%
  mutate(type=fct_relevel(type, "TMetMax", "CTMax")) 


ctmaxtmmax<-ggplot(plotdata, aes(x=type, y=temp))+
  geom_boxplot(size=1.3)+
 geom_jitter(aes(color=stage), size = 3, shape=1, stroke=1.2, width=0.25)  + 
  theme_classic()+ theme(text = element_text(size = 15),legend.position="top")+ 
  xlab("Performance metric")+
  ylab(bquote(Temperature~"(°C)")) +
  scale_color_manual(name="Stage", values=colors)
ctmaxtmmax


ggsave(plot=last_plot(),filename="Toptand CTmaxrigor_stage_minustrial2and outliers.png",  path="Outputs/Statistics/", height=10, width=20, units="cm", dpi=300)

tiff("Outputs/Statistics/Toptand CTmaxrigor_stage_minustrial2and outliers.tiff", width = 20, height = 10, units = "cm", res = 300)
ctmaxtmmax
dev.off()


ggplot(plotdata, aes(x=type, y=temp))+
  geom_violin(size=1.3)+
 #geom_jitter(aes(color=stage), size = 3, shape=1, stroke=1.2)  + 
  theme_classic()+ theme(text = element_text(size = 15),legend.position="top")+ 
  xlab("Performance metric")+
  ylab(bquote(Temperature~"(°C)")) +
  scale_color_manual(name="Stage", values=colors)+
...
#ggsave(plot=last_plot(),filename="Topt_stage_minustrial2and outliers.jpg",  path="Outputs/Statistics/", height=10, width=17, units="cm")
...
mean(dat2b$tempcorr)
sd(dat2b$tempcorr)/length(dat2b$tempcorr)
...
report<-reporter::create_report(file_path = "Outputs/Statistics/Topt_stage_pairwise", output_type = "DOCX",orientation = "portrait",font="Times") %>%
  add_content(table1) %>%
  add_content(table2)
write_report(report)
...
plottst<-ggplot(dat2b, aes(x = stage))+ 
  geom_point(aes(y = tempcorr, color=stage), size = 3, shape=1, stroke=1.2)  + 
  geom_point(aes(y = mean, color=stage),color = "black", size = 3, data = averages, position=position_nudge(x=0.3)) + 
  geom_errorbar(aes(x=stage, ymin = lwr3, ymax = upr3, group=stage), color = "black", data = averages, inherit.aes = FALSE, position=position_nudge(x=0.3), width=0.2) +
#geom_point(aes(y = median),color = "red", size = 3, data = averages, position=position_nudge(x=0.2))+ #plotting the median
  theme_classic()+ theme(text = element_text(size = 15),legend.position="none")+ 
  xlab("Stage")+
  ylab(bquote(T[MetMax]~"(°C)")) +
  scale_color_manual(name="Stage", values=colors)
plottst
ggsave(plot=last_plot(),filename="Topt_stage_minustrial2and outliers.jpg",  path="Outputs/Statistics/", height=10, width=17, units="cm")
...
sjPlot::tab_model(t1b3, show.se = T, show.ci = F, show.re.var = F, show.intercept = T, 
                  show.icc = FALSE, show.ngroups = F, show.r2 = T,
                  dv.labels = c("Thermal optima per mass"), 
...
plottwg<-ggplot(datf, aes(x = weight))+ 
  geom_point(aes(y = tempcorr, color=stage), size = 3, shape=1, stroke=1.2)  + 
  geom_smooth(aes(y=tempcorr), method = "lm", se = TRUE, color="black")+
  theme_classic()+ theme(text = element_text(size = 15),legend.position="top")+ 
  xlab("Mass (mg)")+
  ylab(bquote(T[MetMax]~"(°C)")) +
  scale_color_manual(name="Stage", values=colors)
...
#  ylab("Thermal optima (Celsius)")
#r10plot
ggsave(plot=last_plot(),filename="Topt_weight_perstage_minustrial2andOutliers.jpg",  path="Outputs/Statistics/",height=10, width=20, units="cm")
...
adults<-dat2b%>% filter(stage=="Female" | stage=="Male")
mean(adults$weight)
mean(adults$ropt2)
...
```{r}
mean(dat2b$ropt2)
sd(dat2b$ropt2)/length(dat2b$ropt2)
...
# check Chads code
plotrst<-ggplot(datf, aes(x=stage, y=ropt2, color=stage))+
  geom_point(shape=1, size=3, stroke=1.2)+
  scale_fill_brewer(palette="Set1")+
  stat_summary(geom="point", fun=mean, size=3, color="black", position=position_nudge(x=0.3))+
  stat_summary(geom="errorbar", fun.data=mean_se, width=0.1, position=position_nudge(x=0.3), color="black")+
  theme_classic()+ theme(text = element_text(size = 15),legend.position="none")+ 
  xlab("Stage")+
  ylab(bquote(VO2[Max]~ ~ (mg ~ min^"-1"))) +
  scale_color_manual(name="Stage", values=colors)+
   annotate("text",
           x = c(1.3,2.3,3.3,4.3, 5.3),
           y = c(3e-7, 3.5e-7, 3.8e-7, 4.2e-7, 4.3e-7),
           label = c("a", "b", "b", "c", "c"),
           family = "", fontface = 2, size=5)
plotrst


ggsave(plot=last_plot(),filename="Ropt_stage_perstage_minustrial2andOutliers.jpg",  path="Outputs/Statistics/", height=10, width=17, units="cm")
```
...
sjPlot::tab_model(t1b5, show.se = T, show.ci = F, show.re.var = F, show.intercept = T, 
                  show.icc = FALSE, show.ngroups = F, show.r2 = T,
                  dv.labels = c("Rate optima per mass"), 
...
plotrwg<-ggplot(datf, aes(x = weight))+ 
 ...
 # ylab("Rate optima (Oxygen (mg) / time (min))")+
  scale_color_manual(name="Stage", values=colors)
plotrwg
ggsave(plot=last_plot(),filename="Ropt_weight_perstage_minustrial2andOutliers.jpg",  path="Outputs/Statistics/",height=10, width=20, units="cm")
...
```
##### Checking saling exponent for each stage

```{r}
protos<-dat2b%>%
  filter(stage=="Protonymph")
p1<-lm(log(ropt2)~log(weight), data=protos)
summary(p1)

deutos<-dat2b%>%
  filter(stage=="Deutonymph")
d1<-lm(log(ropt2)~log(weight), data=deutos)
summary(d1)

trito<-dat2b%>%
  filter(stage=="Tritonymph")
t1<-lm(log(ropt2)~log(weight), data=trito)
summary(t1)

males<-dat2b%>%
  filter(stage=="Male")
m1<-lm(log(ropt2)~log(weight), data=males)
summary(m1)

females<-dat2b%>%
  filter(stage=="Female")
f1<-lm(log(ropt2)~log(weight), data=females)
summary(f1)
...
round(confint(t1b5, level=0.95), digits=2 )
summary(t1b5)
sjPlot::tab_model(t1b5, show.se = T, show.ci = T, show.re.var = F, show.intercept = T, 
                  show.icc = FALSE, show.ngroups = F, show.r2 = T,
...
r10plot<-ggplot(datf, aes(x = log(weight)))+ 
  ...
  theme_classic()+ theme(text = element_text(size = 15))+ 
  xlab("log(Weight)")+
  ylab(bquote(VO2[Max]~"(°C)"))
  #ylab("log(Rate optima")
r10plot
#ggsave(plot=last_plot(),filename="Ropt_weight_minustrial2and outliers.jpg",  path="Outputs/Statistics/")

plotlogrwg<-ggplot(datf, aes(x = weight))+ 
  geom_point(aes(y = log(ropt2), color=stage), size = 3, stroke=1.2, shape=1) + #scale_color_gradient(low="#9ecae1", high="#08306b")  + 
  geom_line(aes(y = .fitted), data = predicted.r10, size = 2) + 
  geom_ribbon(aes(ymin = lwr, ymax = upr), data = predicted.r10, alpha = 0.5) + 
  theme_classic()+ theme(text = element_text(size = 15),legend.position="top")+ 
  xlab("ln(Mass) (mg)")+
  ylab(bquote(ln(VO2[Max])~ ~ (mg  ~ min^"-1"))) +
  #ylab("log(Rate optima)")+
  scale_color_manual(name="Stage", values=colors)
  

plotlogrwg
ggsave(plot=last_plot(),filename="Ropt_logweight_perstage_minustrial2andOutliers.jpg",  path="Outputs/Statistics/",height=10, width=20, units="cm")


tiff("Outputs/Statistics/Ropt_logweight_perstage_minustrial2andOutliers.tiff", width = 20, height = 10, units = "cm", res = 300)
plotlogrwg
dev.off()

plotlogrwg<-ggplot(datf, aes(x = log(weight)))+ 
  geom_point(aes(y = log(ropt2), color=stage), size = 3, stroke=1.2, shape=1) + #scale_color_gradient(low="#9ecae1", high="#08306b")  + 
  geom_line(aes(y = .fitted), data = predicted.r10, size = 2) + 
  geom_ribbon(aes(ymin = lwr, ymax = upr), data = predicted.r10, alpha = 0.5) + 
  theme_classic()+ theme(text = element_text(size = 15),legend.position="top")+ 
  xlab("log(Mass)")+
  ylab(bquote(log(VO2[Max]))) +
  scale_color_manual(name="Stage", values=colors)+
  annotate("text",
           x = 0,
           y = -15.45,
           label = c("y= 0.19(±0.02)x –14.84(±0.02), p<0.001 "),
           family = "", fontface = 3, size=5)
  
plotlogrwg
ggsave(plot=last_plot(),filename="Ropt_logweight_perstage_minustrial2andOutliers.jpg",  path="Outputs/Statistics/", width=20, height=10, units="cm", dpi=300)
...
## Looking at the Mass Corrected Metabolic Rate (MCMR)
### Checking if the slope (escaling exponent) varies per stage
```{r}
mcmr1<-lm(log(ropt2)~stage*log(weight), data=dat2b)
summary(mcmr1)
car::Anova(mcmr1)
```

The interaction is no contributing to explain the variance in the data, therefore, the scalling exponent does not differ between stages. Since the interaction is non-significant, then I can use the residuals of the log-log plot where I got the scalling factor

### Getting and comparing residuals

```{r}
# Model used for the scalling factor
t1b5<-lm(log(ropt2)~log(weight), data=dat2b)
summary(t1b5)

#int<-t1b5$coefficients[1]

int<-mean(log(dat2b$ropt2)) #average of the Vo2 max of the log values. Use this one to make it more biological

rownames(dat2b)<-c(dat2b$ind)
resids<-data.frame(cbind(resid=residuals(t1b5), ind=as.numeric(names(residuals(t1b5)))))
...
#Testing if the residuals varied per group
## For raw value
anov<-aov(resid~stage, data=resids2)
summary(anov)
rawlm<-lm(resid~stage, data=resids2)
summary(rawlm)
drop1(rawlm, test="Chisq")

## For mean-added values (use this one)
anov<-aov(residadd~stage, data=resids2)
summary(anov)
```

The anova is non-significant. This means that per unit of mass, all stages have the same metabolic rate. If we have 2 mg of cells of a protonymph, they have the same metabolic rate than 2 mg of cells of an adult. That explains why the raw metabolic rate increases with stage, because the tissues of each stage use the same metabolic rate.


### Plotting the raw residual values
```{r}
# Plotting the residuals 
res<-ggplot(aes(x=stage, y=resid, color=stage), data=resids2)+
    geom_boxplot()+
  geom_jitter(shape=1, size=3, stroke=1.2)+
  theme_classic()+ theme(text = element_text(size = 15),legend.position="none")+ 
  xlab("Stage")+
   ylab(bquote(Mass~corrected~VO2[Max]))  +
  scale_color_manual(name="Stage", values=colors)
res
#ggsave(plot=last_plot(),filename="MCMR_perstage_minustrial2andOutliers_boxplot.jpg",  path="Outputs/Statistics/", height=10, width=17, units="cm")




#plotmcmr<-ggplot(resids2, aes(x=stage, y=resid, color=stage))+
#  geom_point(shape=1, size=3, stroke=1.2)+
#  scale_fill_brewer(palette="Set1")+
...
#  theme_classic()+ theme(text = element_text(size = 15),legend.position="none")+ 
#  xlab("Stage")+
#  ylab(bquote(VO2[Max]~(mgO[2]~min^-1))) +
#  scale_color_manual(name="Stage", values=colors)
#plotmcmr
...
averages<-resids2 %>%
  group_by(stage) %>%
  summarize(mean=mean(resid), sd=sd(resid), n=n(), sem=sd/sqrt(n), median=median(resid))

averages
averages$lwr3 <- with(averages, mean -  sem) #Using just the standard error
averages$upr3 <- with(averages, mean + sem)

exp(mean(resids2$resid))
exp(sd(resids2$resid))


plotmcmr<-ggplot(resids2, aes(x = stage))+ 
  geom_point(aes(y = resid, color=stage), size = 3, shape=1, stroke=1.2)  + 
  geom_point(aes(y = mean, color=stage),color = "black", size = 3, data = averages, position=position_nudge(x=0.2)) + 
  geom_errorbar(aes(x=stage, ymin = lwr3, ymax = upr3, group=stage), color = "black", data = averages, inherit.aes = FALSE, position=position_nudge(x=0.2), width=0.2) +
#geom_point(aes(y = median),color = "red", size = 3, data = averages, position=position_nudge(x=0.2))+ #plotting the median
  theme_classic()+ theme(text = element_text(size = 15),legend.position="none")+ 
...
### Plotting the residuals+mean values 
```{r}
# Plotting the residuals + mean
res2<-ggplot(aes(x=stage, y=residadd, color=stage), data=resids2)+
    geom_boxplot()+
  geom_jitter(shape=1, size=3, stroke=1.2)+
  theme_classic()+ theme(text = element_text(size = 15),legend.position="none")+ 
  xlab("Stage")+
  ylab(bquote(Mass~corrected~VO2[Max]~(log(mgO[2]~min^-1)))) +
  scale_color_manual(name="Stage", values=colors)
res2
ggsave(plot=last_plot(),filename="MCMR_perstage_minustrial2andOutliers_boxplot.jpg",  path="Outputs/Statistics/", height=12, width=17, units="cm")



#plotmcmr<-ggplot(resids2, aes(x=stage, y=residadd, color=stage))+
#  geom_point(shape=1, size=3, stroke=1.2)+
#  scale_fill_brewer(palette="Set1")+
#  stat_summary(geom="point", fun=mean, size=3, color="black", position=position_nudge(x=0.2))+
#  stat_summary(geom="errorbar", fun.data=mean_se, width=0.1, position=position_nudge(x=0.2), color="black")+
...
averages<-resids2 %>%
  group_by(stage) %>%
  summarize(mean=mean(residadd), sd=sd(residadd), n=n(), sem=sd/sqrt(n), median=median(residadd))

averages
averages$lwr3 <- with(averages, mean -  sem) #Using just the standard error
averages$upr3 <- with(averages, mean + sem)

plotmcmr2<-ggplot(resids2, aes(x = stage))+ 
  geom_point(aes(y = residadd, color=stage), size = 3, shape=1, stroke=1.2)  + 
  geom_point(aes(y = mean, color=stage),color = "black", size = 3, data = averages, position=position_nudge(x=0.2)) + 
  geom_errorbar(aes(x=stage, ymin = lwr3, ymax = upr3, group=stage), color = "black", data = averages, inherit.aes = FALSE, position=position_nudge(x=0.2), width=0.2) +
#geom_point(aes(y = median),color = "red", size = 3, data = averages, position=position_nudge(x=0.2))+ #plotting the median
  theme_classic()+ theme(text = element_text(size = 15),legend.position="none")+ 
  xlab("Stage")+
  ylab(bquote(atop(Mass~corrected~VO2[Max]~phantom(),(log(mgO[2]~min^-1))))) +
  scale_color_manual(name="Stage", values=colors)
plotmcmr2

ggsave(plot=last_plot(),filename="MCMR_perstage_minustrial2andOutliers_meanse.jpg",  path="Outputs/Statistics/", height=12, width=17, units="cm")
```

# Using the back-rtransormed residuals+mean (use this one)
```{r}
# Reversing the log transformation
plotmcmr3<-ggplot(resids2, aes(x = stage))+ 
  geom_point(aes(y = exp(residadd), color=stage), size = 3, shape=1, stroke=1.2)  + 
  geom_point(aes(y = exp(mean), color=stage),color = "black", size = 3, data = averages, position=position_nudge(x=0.3)) + 
  geom_errorbar(aes(x=stage, ymin = exp(lwr3), ymax = exp(upr3), group=stage), color = "black", data = averages, inherit.aes = FALSE, position=position_nudge(x=0.3), width=0.2) +
#geom_point(aes(y = median),color = "red", size = 3, data = averages, position=position_nudge(x=0.2))+ #plotting the median
  theme_classic()+ theme(text = element_text(size = 15),legend.position="none")+ 
  xlab("Stage")+
  ylab(bquote(Mass~corrected~VO2[Max]~(mgO[2]~min^-1))) +
  scale_color_manual(name="Stage", values=colors)
plotmcmr3


mean(exp(resids2$residadd))
round(sd(exp(resids2$residadd)), digits=10)

ggsave(plot=last_plot(),filename="MCMR_perstage_minustrial2andOutliers_meanse_reverselog.jpg",  path="Outputs/Statistics/", height=12, width=17, units="cm")
...
### Getting the composite figure for VO2max per stage

```{r}
# Raw residuals
vo2<-ggarrange(plotrst,plotmcmr, nrow=2, labels=c("A", "B"), font.label = list(size = 18, color = "black", face = "bold", family = NULL), align="v")
vo2
ggsave(plot=vo2,filename="VO2maxs_rawresid.jpg",  path="Outputs/Statistics/",width=7, height=10, units="in")


# plus the mean of the log values
vo2<-ggarrange(plotrst,plotmcmr2, nrow=2, labels=c("A", "B"), font.label = list(size = 18, color = "black", face = "bold", family = NULL), align="v")
vo2

ggsave(plot=vo2,filename="VO2maxs.jpg",  path="Outputs/Statistics/",width=7, height=10, units="in")

# backtransformed mean
vo2_exp<-ggarrange(plotrst,plotmcmr3, nrow=2, labels=c("A", "B"), font.label = list(size = 18, color = "black", face = "bold", family = NULL), align="v")
vo2_exp

ggsave(plot=vo2_exp,filename="VO2maxs_backtransformed.jpg",  path="Outputs/Statistics/",width=7, height=10, units="in")
...
mean(dat2b$slope)
sd(dat2b$slope)/length(dat2b$slope)
sd(dat2b$slope)
median(dat2b$slope)
```
...
colors<-c("#ff7f00", "#e41a1c", "#a65628", "#984ea3", "#377eb8")
plotest<-ggplot(datf, aes(x = stage))+ 
  geom_point(aes(y = slope, color=stage), size = 3, shape=1, stroke=1.2)  + 
  geom_point(aes(y = mean, color=stage),color = "black", size = 3, data = averages, position=position_nudge(x=0.3)) + 
  geom_errorbar(aes(x=stage, ymin = lwr3, ymax = upr3, group=stage), color = "black", data = averages, inherit.aes = FALSE, position=position_nudge(x=0.3), width=0.2) +
#geom_point(aes(y = median),color = "red", size = 3, data = averages, position=position_nudge(x=0.2))+ #plotting the median
  ...
  scale_color_manual(name="Stage", values=colors)
plotest
ggsave(plot=last_plot(),filename="energy_stage_minustrial2andoutliers.jpg",  path="Outputs/Statistics/",height=10, width=17, units="cm")
confint(t1b3)
coefTable <- summary(t1b3, ddf = "Kenward-Roger")$coefficients
coefTable
...
sjPlot::tab_model(t1b3, show.se = T, show.ci = F, show.re.var = F, show.intercept = T, 
                  show.icc = FALSE, show.ngroups = F, show.r2 = T,
                  dv.labels = c("Activation energy per mass"), 
...
colors<-c("#ff7f00", "#e41a1c", "#a65628", "#984ea3", "#377eb8")
plotewg<-ggplot(datf, aes(x = weight))+ 
  geom_point(aes(y = slope, color=stage), size = 3, shape=1, stroke=1.2)  + 
  geom_smooth(aes(y=slope), method = "lm", se = FALSE, color="black")+
  theme_classic()+ theme(text = element_text(size = 15),legend.position="top")+ 
  xlab("Mass (mg)")+
  ylab("Activation energy (eV)")+
  scale_color_manual(name="Stage", values=colors)
...
plotewg
ggsave(plot=last_plot(),filename="energy_weight_minustrial2andoutliers.jpg",  path="Outputs/Statistics/",height=10, width=20, units="cm")
...
colors<-c("#ff7f00", "#e41a1c", "#a65628", "#984ea3", "#377eb8")
# check Chads code
means<-ggplot(meansplot, aes(x=period, y=means, color=stage))+
...
# check Chads code
colors<-c("#ff7f00", "#e41a1c", "#a65628", "#984ea3", "#377eb8")
maxs<-ggplot(maxsplot, aes(x=period, y=maxs, color=stage))+
  geom_jitter(shape=1, size=3, stroke=1.2, width=0.1)+
  scale_fill_brewer(palette="Set1")+
  stat_summary(geom="point", fun=mean, size=3, color="black", position=position_nudge(x=0.2))+
  stat_summary(geom="errorbar", fun.data=mean_se, width=0.1, position=position_nudge(x=0.2), color="black")+
...
### Microenvironment figures


This part of the code requires an object from the "Environment vs performance" script
```{r}
load("Outputs/Env vs performance.RData")
fig<-ggarrange( maxs+theme(text = element_text(size = 14),legend.position="top"),fems+theme(text = element_text(size = 14)), ncol=1, labels=c("A", "B"), nrow=2, heights=c(1,2), font.label = list(size = 16 ))
fig
ggsave(plot=fig,filename="figenv.jpg",  path="Outputs/Statistics/",width=7, height=10, units="in")


```

# Making prediction and example plots for dissertation

## Making the prediction plots microenv vs trace
...
#write.csv(dummy,"Outputs/Statistics/Dummydataforprecplots.csv")

dummy<-read.csv("Outputs/Statistics/Dummydataforprecplots.csv")
...
coeff<-50
p1<-ggplot()+
  geom_histogram(dummy, mapping=aes(x=micro),fill="#a6cee3", alpha=1, binwidth=0.5)+
  ...
  scale_x_continuous( breaks=NULL,limits=c(-0.50,12))+
  scale_y_continuous("Environment's temperature frequency", sec.axis = sec_axis(~ ./coeff, name = "Thermal performance", breaks=NULL), breaks=NULL)+
  geom_segment(aes(x = max(dummy$micro), y = 100, xend = 9.55, yend = 100), linetype="dashed", size=1.2, color="red")+
  #geom_segment(aes(x = mean(dummy$micro), y = 100, xend = 9.55, yend = 100), linetype="dashed", size=1.2, color="red") +
  geom_label_repel(label="TMetMax", aes(x=9.55, y=100), nudge_x = 1, nudge_y = 2, arrow = arrow(length = unit(0.015, "npc")),min.segment.length = 0 )
p1
...
p2<-ggplot()+
  ...
  theme(text = element_text(size = 15), plot.title = element_text(size = (12)), strip.text.x = element_blank(),panel.spacing = unit(7, "mm"))+
  xlab("Temperature (Celsius)")+
   scale_x_continuous( breaks=NULL, limits=c(-0.5,12))+
  scale_y_continuous("Environment's temperature frequency", sec.axis = sec_axis(~ ./coeff, name = "Thermal performance" , breaks=NULL), breaks=NULL)+
  geom_segment(aes(x = max(dummy$micro), y = 100, xend = 1.05, yend = 100), linetype="dashed", size=1.2, color="red")+
 #geom_segment(aes(xend = mean(dummy$micro), y = 100, x = 1.05, yend = 100), linetype="dashed", size=1.2, color="red")+
  geom_label_repel(label="TMetMax", aes(x=1.05, y=100), nudge_x = 1, nudge_y = 2, arrow = arrow(length = unit(0.015, "npc")),min.segment.length = 0 )
p2  
ggsave(plot=last_plot(),filename="Predictionplot2.svg",  path="Outputs/Statistics/",width=7, height=7, units="in")


predicts<-ggarrange(p1,p2, ncol=2, labels=c("A", "B"), font.label = list(size = 18, color = "black", face = "bold", family = NULL))
predicts

ggsave(plot=last_plot(),filename="Predictionplotcombined.svg",  path="Outputs/Statistics/",width=20, height=12, units="cm")
ggsave(plot=last_plot(),filename="Predictionplotcombined.jpg",  path="Outputs/Statistics/",width=20, height=12, units="cm")


```

## Making the explanatory plot labeled for Manuscript
...
#write.csv(data_long, "exampleplotdata.csv")

data_long<-read.csv("exampleplotdata.csv")
...
vo2<-expression(VO2[Max])
tmm<-expression(T[MetMax])
ct<-expression(CT[Max])

library(ggrepel)
p3<-ggplot(data_long, aes(value)) +
  geom_density(size=1.2)+
  theme_classic()+
  theme(text = element_text(size = 15), plot.title = element_text(size = (12)), strip.text.x = element_blank(),panel.spacing = unit(7, "mm"))+
  xlab("Temperature (Celsius)")+
  scale_x_continuous( breaks=NULL,limits=c(0.6,1), expand=c(0,0))+
  scale_y_continuous("Performance (Oxygen consumed, VO2)", breaks=NULL, expand=c(0,0))+
  geom_segment(aes(x = 0.6, y =4.35, xend = 0.9, yend = 4.35), linetype="dashed", size=1.2, color="#02818a")+ #VO2
  geom_segment(aes(x =0.9, y = 0, xend = 0.9, yend =4.35), linetype="dashed", size=1.2, color="#d94801") + #TMetMax
  geom_segment(aes(x =0.6, y = 0.34, xend = 0.9, yend =4.35), linetype="dashed", size=1.2, color="#810f7c")+ #Activation energy
  geom_segment(aes(x =1, y = 0, xend = 1, yend =0.8), linetype="dashed", size=1.2, color="#a50f15") +#CTmax
  geom_text(label=vo2, aes(x=0.75, y=4.3), vjust=0, nudge_y=-0.3, color="#02818a", size=7) + #VO2
  geom_text(label=tmm, aes(x=0.95, y=2.175), hjust=0, nudge_x=-0.11, color="#d94801", size=7)+ #TMetMax
  geom_text(label="E", aes(x=0.75, y=2.175), hjust=0, nudge_x=-0.05, color="#810f7c", size=7) + #Activation energy
  geom_text(label=ct, aes(x=1, y=0.4), hjust=0, nudge_x=-0.05, color="#a50f15", size=7)
p3


ggsave(plot=last_plot(),filename="Exampleplot.svg",  path="Outputs/Statistics/",width=17, height=10, units="cm")
ggsave(plot=last_plot(),filename="Exampleplot.png",  path="Outputs/Statistics/",width=17, height=10, units="cm")



```

# Making composite figures for manuscript
```{r}
plotrst #VO2Max
plotmcmr3 #Mass corrected VO2max
plottst #TMetMax
plotest #Activation Energy

initials<-c("P", "D", "T", "M", "F")
# backtransformed mean
fc1<-ggarrange(plotrst+scale_x_discrete(labels= initials),
               plottst+scale_x_discrete(labels= initials),
               plotmcmr3+scale_x_discrete(labels= initials),
               plotest+scale_x_discrete(labels= initials), 
               nrow=2, ncol=2, labels=c("A", "C", "B", "D"), align="v", font.label = list(size = 18, color = "black", face = "bold", family = NULL))
fc1

ggsave(plot=fc1,filename="Fig1comp.jpg",  path="Outputs/Statistics/",width=10, height=10, units="in", dpi=300)

tiff("Outputs/Statistics/Fig1comp.tiff", width = 10, height = 10, units = "in", res = 300)
fc1
dev.off()
