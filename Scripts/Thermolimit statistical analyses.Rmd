...
plottwg<-ggplot(datf, aes(x = weight))+ 
  geom_point(aes(y = tempcorr, color=stage), size = 3, shape=1, stroke=1.2)  + 
  geom_smooth(aes(y=tempcorr), method = "lm", se = FALSE, color="black")+
...
The interaction is no contributing to explain the variance in the data. Since the interaction is non-significant, then I can use the residuals of the log-log plot where I got the scalling factor

```{r}
t1b5<-lm(log(ropt2)~log(weight), data=dat2b)
summary(t1b5)

log(mean(dat2b$ropt2))
mean(log(dat2b$ropt2)) #ASK Kristi which one to use
...
res<-ggplot(aes(x=stage, y=resid, color=stage), data=resids2)+
  ...
  scale_color_manual(name="Stage", values=colors)
res
ggsave(plot=last_plot(),filename="MCMR_perstage_minustrial2andOutliers.jpg",  path="Outputs/Statistics/", height=10, width=17, units="cm")
