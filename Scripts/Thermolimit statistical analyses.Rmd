...
t1b3<-lmer(tempcorr~weight+(1|date), data=dat2b, REML=TRUE)

step(t1b3)

t1b3<-lm(tempcorr~weight, data=dat2b, REML=TRUE)

drop1(t1b3, test="Chisq")
...
```{r}
check_assumptions(t1b3)
hist(t1b3$residuals, main = "Residual Histogram")
ols_test_normality(t1b3) #Kolmogorov: data is normal
```
...
# Plot with prediction trends, confidence intervals and data
r10plot<-ggplot(dat2b, aes(x = weight))+ 
...
r10plot<-ggplot(dat2b, aes(x = weight))+ 
  ...
  ylab("Rate optima (Oxygen (mg/L) / time (min))")
r10plot
ggsave(plot=last_plot(),filename="Ropt_weight_perstage_minustrial2andOutliers.jpg",  path="Outputs/Statistics/")
