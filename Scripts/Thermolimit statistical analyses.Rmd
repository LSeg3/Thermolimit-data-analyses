...
means<-means %>%
  mutate(emmean=round(emmean, digits=10),
         SE=round(SE, digits=10),
         lower.CL=round(lower.CL, digits=10),
         upper.CL=round(upper.CL, digits=10))
...
plottwg<-ggplot(datf, aes(x = weight))+ 
  geom_point(aes(y = tempcorr, color=stage), size = 3, shape=1, stroke=1.2)  + 
  geom_smooth(aes(y=tempcorr), method = "lm", se = TRUE, color="black", size=2, alpha=0.5)+
...
```
# COMPARING ROPT OR MAX METABOLIC RATE
## Checking for outliers
```{r}
#Comparing boxplots for all data vs separated by sex: points represent data values that alie outside 1.5xIQR (Inter Quartile Range or the ifference between the 75th and 25th quantiles)
##Rate optima
...
sample2
```
#checking weight and ropt for adults only
```{r}
means<-dat2b %>%
  filter(stage %in% c("Male", "Female"))
mean(means$ropt2)
mean(means$weight)
```
#### Looking at the results
...
contrasts<-as.data.frame(pair$contrasts)
contrasts<-contrasts %>%
  mutate(estimate=round(estimate, digits=10),
         SE=round(SE, digits=10),
         t.ratio=round(t.ratio, digits=2),
         p.value=round(p.value, digits=3))
table1<-  create_table(means)
...
plottwg<-ggplot(datf, aes(x = weight))+ 
  geom_point(aes(y = ropt2, color=stage), size = 3, shape=1, stroke=1.2)  + 
  geom_smooth(aes(y=ropt2), method = "lm", se = TRUE, color="black", size=2, alpha=0.5)+
  theme_classic()+ theme(text = element_text(size = 15),legend.position="top")+ 
  xlab("Mass (mg)")+
  ylab(bquote(VO2[Max]~(mgO[2]~min^-1)))+
  scale_color_manual(name="Stage", values=colors)
...
plotlogrwg
ggsave(plot=last_plot(),filename="Ropt_logweight_perstage_minustrial2andOutliers.jpg",  path="Outputs/Statistics/", width=20, height=10, units="cm")
```
```{r}
rm(list = ls())
```
...
plotmcmr3<-ggplot(resids2, aes(x = stage))+ 
  ...
  geom_point(aes(y = exp(mean), color=stage),color = "black", size = 3, data = averages, position=position_nudge(x=0.2)) + 
  geom_errorbar(aes(x=stage, ymin = exp(lwr3), ymax = exp(upr3), group=stage), color = "black", data = averages, inherit.aes = FALSE, position=position_nudge(x=0.2), width=0.2) +
#geom_point(aes(y = median),color = "red", size = 3, data = averages, position=position_nudge(x=0.2))+ #plotting the median
  theme_classic()+ theme(text = element_text(size = 15),legend.position="none")+ 
  xlab("Stage")+
  ylab(bquote(atop(Mass~corrected~VO2[Max]~phantom(),(mgO[2]~min^-1)))) +
  scale_color_manual(name="Stage", values=colors)
plotmcmr3
...
text(x=1:length(cooksd)+1, y=cooksd, labels=ifelse(cooksd>4*mean(cooksd, na.rm=T),names(cooksd),""), col="red")  # add labels
#Removing all influential points
influential <- as.numeric(names(cooksd)[(cooksd > (3*mean(cooksd)))]) #Identifying row number for the outliers
influential
#removing the top most influential points: this is better, removes the 3 from the boxplot
top_x_outlier <- 2
influential <- as.numeric(names(sort(cooksd, decreasing = TRUE)[1:top_x_outlier]))
...
plotest<-ggplot(datf, aes(x = stage))+ 
  geom_point(aes(y = slope, color=stage), size = 3, shape=1, stroke=1.2)  + 
  geom_point(aes(y = mean),color = "black", size = 3, data = averages, position=position_nudge(x=0.2)) + 
  geom_errorbar(aes(x=stage, ymin = lwr3, ymax = upr3, group=stage), color = "black", data = averages, inherit.aes = FALSE, position=position_nudge(x=0.2), width=0.2) +
#geom_point(aes(y = median),color = "red", size = 3, data = averages, position=position_nudge(x=0.2))+ #plotting the median
  theme_classic()+ theme(text = element_text(size = 15), legend.position="none")+ 
  xlab("Stage")+
...
plotewg<-ggplot(datf, aes(x = weight))+ 
  geom_point(aes(y = slope, color=stage), size = 3, shape=1, stroke=1.2)  + 
  geom_smooth(aes(y=slope), method = "lm", se = TRUE, color="black", size=2, alpha=0.5)+
  theme_classic()+ theme(text = element_text(size = 15),legend.position="top")+ 
  xlab("Mass (mg)")+
  ylab("Activation energy (eV)")+
  scale_color_manual(name="Stage", values=colors)
...
p3<-ggplot(data_long, aes(value)) +
  ...
  geom_text(label="VO2max", aes(x=0.75, y=4.3), vjust=0, nudge_y=-0.3, color="#02818a", size=7) + #VO2
  geom_text(label="TMetMax", aes(x=0.95, y=2.175), hjust=0, nudge_x=-0.13, color="#d94801", size=7)+ #TMetMax
  geom_text(label="E", aes(x=0.75, y=2.175), hjust=0, nudge_x=-0.05, color="#810f7c", size=7)+ #Activation energy
 geom_text(label="CTmax", aes(x=1, y=0.4), hjust=0, nudge_x=-0.06, color="#a50f15", size=7) #CTmax
...
```
library(magick)
A<-"Plots without outliers 2022/sex2-noOutliers.jpeg"
B<-"C:/Users/laus1/OneDrive - University of Nebraska-Lincoln/UNL/Courses/Ecological statistics/Segura_Final project final analyses/exposure.png"
URL<-c(A,B)
imgs<-image_read(URL)
imgs2<-image_montage(imgs, title="1x2")