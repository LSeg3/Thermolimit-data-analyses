...
ggplot(ox2, aes(x=logtime, y=oxymgl,group=ind, color=stage))+
  geom_line()+
  theme_classic()+
  theme(text = element_text(size = 15))+
  ylim(8.5,12) +
...
ggplot(ox2, aes(x=logtime)) +
  geom_line(aes(y=oxymgl, group=ind, color=ind))+
  geom_line(aes(y=tempcorr/2.5), color="red")+
  theme_classic(size=20)+
  facet_wrap(~ind)+
  scale_y_continuous(name = 'Oxygen (mg/L)', sec.axis = sec_axis(~.*2.5, name = 'Temperature (Celsius)')) +
  theme(
...
```{r}
oxall<-rbind(ox2,ox3,ox4,ox5,ox6,ox7,ox8,ox9,ox10,ox11,ox12,ox13)
oxall<-oxall %>% 
  filter_all(all_vars(!is.infinite(.))) %>%
   filter(!ind%in%c(65,231,121,25,235,82))

#Drop individuals that drowned


write.csv(oxall, "Outputs/oxygenall.csv")

```

## Trying function again after seeing that some individuals did not fit that well with polynomial of 7 degrees


### Polinomial 7 for ind 168
```{r}
...
lines(logtime, predict(fit7, data.frame(x=logtime)), col='grey')
lines(logtime, predict(fit8, data.frame(x=logtime)), col='yellow')
lines(logtime, predict(fit9, data.frame(x=logtime)), col='green', lwd=2)
lines(logtime, predict(fit10, data.frame(x=logtime)), col='orange', lwd=2)
...
# Plotting each predicted function line to the real data
plot(func$logtime, func$oxymgl, pch=19, xlab='Logtime', ylab='Oxygen(mg/l)')
lines(logtime, predict(fit10, data.frame(x=logtime)), col='green', lwd=2)
...
coef10<-as.data.frame(fit10$coefficients)
a0<-coef10[1,1]
a1<-coef10[2,1]
a2<-coef10[3,1]
a3<-coef10[4,1]
...
ggplot(aes(x=logtime, y=oxymgl), data=func)+geom_point()
plot(x, func$oxymgl, pch=19, xlab='Logtime', ylab='Oxygen(mg/l)')
lines(logtime, predict(fit10, data.frame(x=logtime)), col='red')
...
r2 = function(data) {
     fit = lm(oxymgl~poly(logtime, 7, raw=TRUE), data = data)
...
lines(data$logtime, predict(fit, data.frame(logtime=data$logtime)), col='red',lwd=2.5)
dev.off()
     }
...
jpeg(file = mypath, width=20, height=12, units="cm", res=300)
plot(d7f.data$logtime, d7f.data$y1, pch=19, xlab='Logtime', ylab='Oxygen(mg/l)', main=paste("First derivative: Ind ", id), cex.lab=1.5, cex.axis=1.25)
dev.off()

}

map_dfr(dat_list, r4first, .id = "id")
...
mypath <- file.path("Outputs/Second derivative plots/",paste("Individual_", id, ".jpg", sep = ""))
 ...
 jpeg(file = mypath, width=20, height=12, units="cm", res=300)
plot(d7s.data$logtime, d7s.data$y1, pch=19, xlab='Logtime', ylab='Oxygen(mg/l)', main=paste("Second derivative: Ind", id), cex.lab=1.5, cex.axis=1.25)
abline(h=0,col="orange", lty=2, lwd=2.5)
dev.off()
}

map_dfr(dat_list, r4second, .id = "id")
```

## Obtaining the inflection points from the second derivative
...
## 6.  Making combined figures for the function fit, first and second derivatives
...
# Obtaining the maximum rate of oxygen consumption (Ropt)
```{r}
...
topt<-difference_left_join(ropt,oxall, by=c("ind","logtime"), max_dist=0.13) %>%
...
PENDING: cut traces where the data looks weird (e.g. oxygen is being produces instead of consumed)
